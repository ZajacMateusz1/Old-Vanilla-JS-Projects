...
class Game extends UI {
    constructor() {
        ...
        this.difficultes = {
            0: {rows: 8, columns: 8, mines: 10, size: '3rem', boardWidth: '27.5rem'},
            1: {rows: 16, columns: 16, mines: 40, size: '1.8rem', boardWidth: '30.5rem'},
            2: {rows: 16, columns: 30, mines: 99, size: '1.3rem', boardWidth: '40.5rem'}
        };
    ...
    #selectDifficultyLevel () {
        ...
        document.documentElement.style.setProperty('--boardWidth', difficulty.boardWidth);
    ...
    #removeCellEvents() {
        ...
        elements.forEach(element => {
            ...
            element.removeEventListener('contextmenu',this.#cellRightClick.bind(this));
        });
    }
    #findElement(target) {
    ...
    #setCellValue(cell,element) {
        ...
        let mines = 0;
            for(let row = Math.max(cell.x - 1, 0);row<= Math.min(cell.x + 1,this.rows-1);row++){
                for(let col = Math.max(cell.y - 1, 0); col<=Math.min(cell.y+1,this.columns-1); col++) {
    ...
    #cellLeftClick(e) {
        ...
        else if(this.countReveledCells === (this.columns * this.rows - this.mines)){
    ...
    start() {
        this.select.addEventListener('change',this.#selectDifficultyLevel.bind(this));
        this.resetBtn.addEventListener('click',this.#selectDifficultyLevel.bind(this));
        this.playAgainBtn.addEventListener('click',this.#selectDifficultyLevel.bind(this));
